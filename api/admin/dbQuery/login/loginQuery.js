const SELECT_USER_BY_USERNAME = (adp_id) => {
  return `SELECT A.adp_id, A.password, A.otp, A.otp_time, email FROM tbl_user A
...
const INSERT_OTP = (userId, otp) => {
  return `UPDATE tbl_user SET otp= ${otp}, otp_time = now() WHERE adp_id = '${userId}'`;
};
