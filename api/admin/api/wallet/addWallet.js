...
} = require("../../../adp/api/smartMart/debitSmartMart");
...
module.exports = (app) => {
  const {
    ...
    ADD_WALLET_BALANCE,
    CREATE_WALLET,
    INSERT_WALLET_STATEMENT,
  } = require("../../dbQuery/wallet/walletQuery");
  ...
  app.post("/add-wallet-balance", urlencodedParser, async (req, res) => {
    ...
    connection.query(
      ...
      async (error, walletResult, fields) => {
        if (error) return res.sendStatus("401");
        // if (results.length === 0) return res.sendStatus("404");
        if (walletResult && walletResult.length > 0) {
          connection.query(
            ADD_WALLET_BALANCE(adpId, amount),
            async (error, results, fields) => {
              ...
              connection.query(
                ...
                async (error, results, fields) => {
                  ...
                  let credit = amount;
                  connection.query(
                    INSERT_WALLET_STATEMENT(adpId, msg, credit, debit),
                    async (error, results, fields) => {
  ...
  app.post("/debit-sm-balance", urlencodedParser, async (req, res) => {
    let adpId = req.body.adpId;
    let amount = req.body.balance;
    const existingBalance = await getSmartmartBalance(adpId);
    if (!existingBalance) {
      await createSmartMartWallet(adpId);
    }
    await debitSmartMart(adpId, amount);
    res.sendStatus(200);
  });
};
