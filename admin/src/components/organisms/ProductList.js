...
class ProductList extends React.Component {
                ...
                obj.delete = (
                  <DeleteIcon
                    ...
                    onClick={(event) => {
                      ...
                      this.removeProducts(obj.id);
                    }}
                  />
                );
                obj.details &&
                  obj.details.forEach((elm, i) => {
                    elm.delete = (
                      <DeleteIcon
                        className={clsx(classes.deleteIcon, className)}
                        onClick={(event) => {
                    ...
                    elm.free_code_from_calendar = (
                      <DatePicker
                        onChange={(date) => {
                          this.handleCalendarChange(i, "free_code_from", date);
                        }}
                        selected={elm.free_code_from}
                        minDate={new Date()}
                        dateFormat={"dd/MM/yyyy"}
                      />
                    );
                    elm.free_code_to_calendar = (
                      <DatePicker
                        onChange={(date) => {
                          this.handleCalendarChange(i, "free_code_to", date);
                        }}
                        selected={elm.free_code_to}
                        minDate={new Date(elm.free_code_from)}
                        dateFormat={"dd/MM/yyyy"}
                      />
                    );
                  });
  ...
  handleCalendarChange = (index, key, date) => {
    ...
    if (key === "free_code_from") {
      data.updated["free_code_from_calendar"] = (
        <DatePicker
          onChange={(date) => {
            this.handleCalendarChange(index, "free_code_from", date);
          }}
          selected={new Date(date.toDateString("dd/MM/yyy"))}
          minDate={new Date()}
          dateFormat={"dd/MM/yyyy"}
        />
      );
      data.updated["free_code_to_calendar"] = (
        <DatePicker
          onChange={(date) => {
            this.handleCalendarChange(index, "free_code_to", date);
          }}
          selected={""}
          minDate={new Date(date.free_code_from)}
          dateFormat={"dd/MM/yyyy"}
        />
      );
      ...
      data.updated["free_code_to_calendar"] = (
        <DatePicker
          ...
          selected={new Date(date.toDateString("dd/MM/yyy"))}
          minDate={new Date(productList[index]["free_code_from"])}
          dateFormat={"dd/MM/yyyy"}
        />
      );
    }
  ...
  onRowsUpdate = (fromRow, toRow, updated, index) => {
    ...
    if (
      changedKey === "price" ||
      changedKey === "discount" ||
      changedKey === "retail_profit"
    ) {
      updated = validateProductChange(
        currentProduct,
        changedKey,
        updated[changedKey],
        updated
    ...
    this.setState(
      ...
      () => {
        this.props.updateProductsToRows(
          this.props.rowKey,
  ...
  UNSAFE_componentWillReceiveProps = (nextProps) => {
    ...
    if (!addedSample && nextProps.productListData == this.props.productListData)
      return;
  ...
  render() {
    ...
    return (
      <Grid
        container
        spacing={2}
        style={{ border: "1px solid #bdbdbd", marginTop: "2rem" }}
      >
        <Grid
          item
          xs={12}
          ...
          <Grid container spacing={2}>
            ...
            >
              <Select
                ...
                isDisabled={
                  selectedProductType && selectedProductType.label === undefined
                    ? true
                    : false
                }
                onChange={(value) => {
            ...
            >
              <Select
                ...
                isDisabled={
                  selectedCategory && selectedCategory.label === undefined
                    ? true
                    : false
                }
                onChange={(value) => {
...
function validateProductChange(product, key, value, object) {
  ...
  object.actual_price =
    key === "price"
      ? value - product.discount - product.retail_profit
      : key === "discount"
      ? product.price - value - product.retail_profit
