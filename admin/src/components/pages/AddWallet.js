...
class AddWallet extends React.Component {
  ...
  handleAddWallet = () => {
    ...
    addWallet(body).then((response) => {
      alert("Balance addd to Wallet.")
      this.props.updateSelectedAdpSuccess({});
      this.setState({
        amount: 0
      })
...
export default withStyles(styles)(connector(AddWallet));


function addWallet (body) {
  return apiHandler.post("/add-wallet-balance", body);
}